...
process_cal_human_sleep_thresh_3 <- function(data, params, ...) {
  stop("This workunit is now deprecated. Sleep threshold can now be customized via a participant variable in the default human workunit.")
  ...
  ## return two datasets
  datasets <- list(human = human, haldane = ret$haldane[keep])
  files    <- list(human = jsonlite::unbox(base64_rep))
  
  list(datasets = datasets, files = files)
}

human_summary_sleep_thresh_3 <- function(data, params, ...) {
  calrq <- data$haldane
  ...
  if(length(tag_labels) > 0) {
    tag_summary <- pilr.utils.r::fapply_event_tags(calrq, tag_table,
                                                   compute_human_summary_sleep_thresh_3,
                                                   settings)
  }
  ...
  if("Post Meal" %in% event_tags$tags & "Rest" %in% event_tags$tags) {
    ree <- compute_human_summary_sleep_thresh_3(pilr.utils.r::subset_event_tags("Rest", calrq, tag_table),
                                 "Rest", settings, do_tf_correct = FALSE)
    
    mr_pre <- ree$mr * 1080  #18 hours
    
    mr_post  <- compute_human_summary_sleep_thresh_3(pilr.utils.r::subset_event_tags("PostMeal", calrq, tag_table),
                                      "PostMeal", settings,
  ...
  if(nrow(calrq) >= 30) {
    ...
    ei_meas <- pilr.utils.r::get_setting("energy_intake_meas", settings,
                                         required = FALSE) %>%
      pilr.utils.r::safe_numeric()
...
compute_human_summary_sleep_thresh_3 <- function(data, tag_label, settings,
                                  do_tf_correct = TRUE) {
  
  if(tag_label == "TT_SleepNoActivity") {
    data <- subset(data, Activity <= 3)
  }
  ...
  ## obtain all settings we'll need for further computations
  ...
  ## NB: transform for below sum, not included in output
  data$adj_ee <- data$recalc_ee + (-2.17 * nitrogen / 1440)
  ...
  ret$np_rq <- (ret$vco2 - (4.97 * nitrogen)) /
    (ret$vo2 - (5.95 * nitrogen))
  
  ret$protein_ox <- ((nitrogen * 6.26) / 0.966)
  
  ret$cho_ox <- (4.113 * ret$vco2) - (2.907 * ret$vo2) -
    (0.375 * ret$protein_ox)
  
  ret$fat_ox <- (1.689 * ret$vo2) - (1.689 * ret$vco2) -
    (0.324 * ret$protein_ox)
